...
def save_document_contents(
    ...
    with (OUTPUT_DIR / "contenido_documentos.txt").open("w", encoding="utf-8") as f:
        for row in rows:
            f.write(f"Licitacion ID: {row['licitacion_id']}\n")
            f.write(f"Expediente: {row['expediente']}\n")
            f.write(f"Licitacion: {row['title']}\n")
            f.write(f"Tipo documento: {row['document_type_name']}\n")
            f.write(f"Archivo: {row['file_name']}\n")
            f.write(f"URL documento: {row['document_url']}\n")
            f.write(f"Estado extraccion: {row['extract_status']}\n")
            f.write("Contenido:\n")
            f.write((row["text"] or "")[:4000])
            f.write("\n\n")
...
def save_document_timeline(
    ...
    with (OUTPUT_DIR / "timeline_documentos.txt").open("w", encoding="utf-8") as f:
        for r in out:
            f.write(f"Licitacion ID: {r['licitacion_id']}\n")
            f.write(f"Expediente: {r['expediente']}\n")
            f.write(f"Licitacion: {r['title']}\n")
            f.write(f"Fecha publicacion: {r['fecha_publicacion']}\n")
            f.write(f"Documento: {r['documento']}\n")
            f.write(f"Tipo enlace: {r['tipo_enlace']}\n")
            f.write(f"URL documento: {r['url_documento']}\n")
            f.write(f"URL HTML: {r['url_html']}\n")
            f.write(f"URL XML: {r['url_xml']}\n")
            f.write(f"URL PDF: {r['url_pdf']}\n")
            f.write(f"Origen: {r['origen']}\n\n")
...
def extract_document_insights() -> None:
    ...
    with (OUTPUT_DIR / "insights_documentos.txt").open("w", encoding="utf-8") as f:
        for x in insights:
            f.write(f"Licitacion ID: {x['licitacion_id']}\n")
            f.write(f"Expediente: {x['expediente']}\n")
            f.write(f"Documento: {x['document_type_name']}\n")
            f.write(f"URL doc: {x['document_url']}\n")
            f.write(f"Objeto: {x['objeto_contrato']}\n")
            f.write(f"Presupuesto base: {x['presupuesto_base']}\n")
            f.write(f"Valor estimado: {x['valor_estimado']}\n")
            f.write(f"Fecha fin presentacion: {x['fecha_fin_presentacion']}\n")
            f.write(f"Criterios adjudicacion: {x['criterios_adjudicacion']}\n")
            f.write(f"Solvencia: {x['solvencia']}\n")
            f.write(f"CPV texto: {x['cpv_texto']}\n")
            f.write(f"Lugar ejecucion: {x['lugar_ejecucion_texto']}\n\n")
...
def save_outputs(items: list[TenderRecord], docs: list[TenderDocument]) -> None:
    ...
    with (OUTPUT_DIR / "licitaciones.txt").open("w", encoding="utf-8") as f:
        for item in items:
            f.write(f"Licitacion ID: {item.licitacion_id}\n")
            f.write(f"Titulo: {item.title}\n")
            f.write(f"URL: {item.detail_url}\n")
            f.write(f"Expediente: {item.expediente}\n")
            f.write(f"Pais: {item.pais_nombre} ({item.pais_codigo})\n")
            f.write(
                f"Tipo contrato: {tipo_contrato_label(item.tipo_contrato_codigo)} / Subtipo: {item.subtipo_contrato_codigo}\n"
            )
            f.write(
                f"Lugar ejecucion: {item.lugar_ejecucion} ({item.lugar_ejecucion_codigo})\n"
            )
            f.write(f"CPV: {item.cpv_codes}\n")
            f.write(f"Organo: {item.organo_contratacion}\n")
            f.write(f"Organo DIR3: {item.organo_dir3}\n")
            f.write(f"Organo NIF: {item.organo_nif}\n")
            f.write(f"Jerarquia organo: {item.org_hierarchy}\n")
            f.write(f"Estado: {item.estado_codigo}\n")
            f.write(f"Fecha publicacion: {item.fecha_publicacion}\n")
            f.write(
                f"Procedimiento/Sistema: {item.procedimiento_codigo}/{item.sistema_contratacion_codigo}\n"
            )
            f.write(
                f"Importes (estimado/sin IVA/total): {item.importe_estimado}/{item.importe_sin_impuestos}/{item.importe_total}\n"
            )
            f.write(f"Presentacion: {item.forma_presentacion_codigo}\n")
            f.write(
                f"Financiacion UE: {item.financiacion_ue_codigo} ({item.financiacion_ue_nombre})\n"
            )
            f.write(f"Fuente: {item.fuente_publicacion}\n")
            f.write(f"Tipo tramitacion: {item.tipo_tramitacion_codigo}\n")
            f.write(
                f"Plazo presentacion: {item.presentacion_desde} -> {item.presentacion_hasta} {item.presentacion_hora}\n"
            )
            f.write(
                f"Duracion contrato: {item.contrato_duracion} {item.contrato_duracion_unidad}\n"
            )
            f.write(
                f"Ofertas recibidas/PYMES/PYME adjudicada: {item.ofertas_recibidas}/{item.pymes_ofertas}/{item.pyme_adjudicada}\n"
            )
            f.write(f"Adjudicatario: {item.adjudicatario}\n")
            f.write(f"Adjudicatario NIF: {item.adjudicatario_nif}\n")
            f.write(f"Tipos aviso: {item.notice_types}\n")
            f.write(f"Actualizado: {item.updated}\n\n")
    with (OUTPUT_DIR / "documentos_licitacion.txt").open("w", encoding="utf-8") as f:
        for doc in docs:
            f.write(f"Licitacion ID: {doc.licitacion_id}\n")
            f.write(f"URL licitacion: {doc.detail_url}\n")
            f.write(f"Nombre licitacion: {doc.title}\n")
            f.write(f"Expediente: {doc.expediente}\n")
            f.write(f"Estado de la Licitacion: {doc.estado_licitacion}\n")
            f.write(f"Financiacion UE: {doc.financiacion_ue}\n")
            f.write(
                f"Presupuesto base de licitacion sin impuestos: {doc.presupuesto_base_sin_impuestos}\n"
            )
            f.write(f"Valor estimado del contrato: {doc.valor_estimado_contrato}\n")
            f.write(f"Tipo de Contrato: {doc.tipo_contrato}\n")
            f.write(f"Codigo CPV: {doc.codigo_cpv}\n")
            f.write(f"Lugar de ejecucion: {doc.lugar_ejecucion}\n")
            f.write(f"Sistema de contratacion: {doc.sistema_contratacion}\n")
            f.write(f"Procedimiento de contratacion: {doc.procedimiento_contratacion}\n")
            f.write(f"Tipo de tramitacion: {doc.tipo_tramitacion}\n")
            f.write(f"Tipo aviso: {doc.notice_type_code}\n")
            f.write(f"Medio publicacion: {doc.publication_media}\n")
            f.write(f"Fecha publicacion: {doc.publication_date}\n")
            f.write(f"Tipo documento: {doc.document_type_code} ({doc.document_type_name})\n")
            f.write(f"Nombre archivo: {doc.file_name}\n")
            f.write(f"URL documento: {doc.document_url}\n\n")
    ...
    for label, rows_by_type in docs_by_type.items():
        ...
        with (docs_type_dir / f"{slug}.txt").open("w", encoding="utf-8") as f:
            for row in rows_by_type:
                f.write(
                    f"{row['licitacion_id']} | {row['expediente']} | {row['publication_date']} | {row['document_type_name']} | {row['file_name']} | {row['document_url']}\n"
                )

    filter_catalog = build_filter_catalog(rows)
    dictionaries = build_dictionaries(items, docs)
    with (OUTPUT_DIR / "licitaciones.txt").open("a", encoding="utf-8") as f:
        f.write("\n[CATALOGOS_FILTROS]\n")
        for field, values in filter_catalog.items():
            f.write(f"{field}:\n")
            for v in values:
                f.write(f"- {v['value']} -> {v['count']}\n")
            f.write("\n")
        f.write("[DICCIONARIOS]\n")
        for name, rows_dic in dictionaries.items():
            f.write(f"{name}:\n")
            for r in rows_dic:
                f.write(f"- {r['code']} -> {r['descripcion']}\n")
            f.write("\n")
...
def main() -> None:
    ...
    if args.timeline_docs:
        print("Timeline documentos exportado: data/timeline_documentos.json y .txt")
